...
const toggleVideoLike = AsyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiError(
      500,
      "Something went wrong while toggling the video like"
    );
  }
});
...
const toggleCommentLike = AsyncHandler(async () => {
  ...
  try {
    ...
    if (existingLike) {
      ...
      return res
        .status(200)
        .json(new ApiResponse(200, { isLiked: false }, "comment got disliked"));
    } else {
        await Promise.all([
        Like.create({ comment: commentId, likedBy: user }),
...
const toggleTweetLike = AsyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (existingLike) {
      ...
      return res
        .status(200)
        .json(new ApiResponse(200, {}, "you disliked the video"));
    } else {
      ...
      return res
        .status(200)
        .json(new ApiResponse(200, {}, "you liked the videos"));
    }
  } catch (error) {
    ...
    try {
      ...
      return res
        ...
        .json(
          new ApiResponse(
            200,
            { videoLikesCount, allLikesVideos },
